...
class FirebaseLoginView(APIView):
    """
    OPCIONAL — requer o pacote `firebase-admin` (não incluído em
    requirements.txt por padrão, já que o boilerplate usa JWT como
    estratégia de autenticação principal — ver CLAUDE.md). Se o projeto
    realmente precisar de login via Firebase, adicione `firebase-admin`
    ao requirements.txt; caso contrário, esta view sempre retornará 400
    (o import de `firebase_admin` falha dentro de
    `CustomUser.objects.create_user_with_firebase`, que captura o erro).
    """
