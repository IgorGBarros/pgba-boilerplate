...
def _status(agent: Agent, working: bool, tarefa: str = "") -> None:
...
def executar(
    ...
    try:
        ...
        logger.warning("Marketing: saída fora do formato (%s): %s", tarefa, exc)
        raise IAError("A IA respondeu fora do formato combinado — tente de novo.") from exc
    return data
